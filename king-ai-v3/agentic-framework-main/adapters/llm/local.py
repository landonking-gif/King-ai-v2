"""
Local LLM adapter for Ollama and compatible servers.

This adapter integrates with local LLM servers like Ollama, LM Studio, and other
...
class LocalLLMAdapter(LLMAdapter):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            model: Model identifier (e.g., "llama3.1:70b", "mistral:latest", "neural-chat:latest")
            endpoint: Local server endpoint (defaults to OLLAMA_ENDPOINT env var or http://localhost:11434)
        ...
        """
        endpoint = endpoint or os.getenv("OLLAMA_ENDPOINT", self.DEFAULT_ENDPOINT)
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        """
        Generate a completion from local LLM server.
        ...
        try:
            ...
            raise TimeoutError(
                f"Local LLM request timed out (endpoint: {self.endpoint}). "
                f"Ensure server is running: ollama serve"
            )
        except LLMError:
            raise
    ...
    async def stream_complete(
        ...
        **kwargs: Any,
        ...
        """
        Generate a streaming completion from local LLM server.
        ...
        try:
            ...
            raise TimeoutError(
                f"Local LLM request timed out. Ensure server is running: ollama serve"
            )
        except LLMError:
            raise
        except Exception as e:
